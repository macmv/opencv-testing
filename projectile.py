...
V_0     = 1                    # Initial velocity (m/s)
g       = 9.81                 # Acceleration due to gravity (m/s^2)
psi     = 45                   # Launch angle (deg.)
c       = 0.47                 # Drag coefficient (spherical projectile)
r       = 0.1778               # Radius of projectile (m)
m       = 0.140                # Mass of projectile (kg)
...
while(H <= 2.3):
    ...
    Max_H_where = asinh(0.1)
    Max_Q_T = newton(y_s, Max_H_where, y_s_p)
    Max_T = t_s(Max_Q_T)
    H_P = x_s(Max_T)
    # Horizontal range
    R = x_s(Q_T)
